...
from utils.utils_nri import my_softmax, get_offdiag_indices, gumbel_softmax
...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
                 do_prob=0., skip_first=False, ww=False):
...
class RNNDecoder(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, n_hid,
        ...
        self.out_fc2 = nn.Linear(n_hid, n_hid)
        if self.ww:
            self.out_fc3 = nn.Linear(n_hid, n_in_node-1)
        else:
            self.out_fc3 = nn.Linear(n_hid, n_in_node)
    ...
    def single_step_forward(self, inputs, rel_rec, rel_send,
        ...
        if self.ww:
            pred = inputs[..., :-1] + pred
            pred = torch.cat((pred, inputs[..., -1].unsqueeze(-1)), dim=-1)
        else:
            pred = inputs + pred
